...
class IrrationalPanicAgent < SimpleAgent
	attr_accessor :coef_price_decrease, :coef_price_increase, :best_offers, :best_offers_grad,
				  :coef_money_when_buying, :coef_stocks_when_selling, :subscribed_count,
	...
	def self.generateRandomCoef(rand_gen = Random.new, coef_dict = {})
		...
		#coef_stocks_when_selling
		cows_min, cows_max = coef_dict.fetch(:coef_stocks_when_selling, [$coef_stocks_when_selling_min, $coef_stocks_when_selling_max])
		#subscribed_count 
		...
		coefs = {:coef_price_decrease => rand_gen.rand(cpd_min..cpd_max),
				 ...
				 :coef_stocks_when_selling => rand_gen.rand(cows_min..cows_max),
	...
	def self.createInstance(id, password, data)
		...
		inst.coef_stocks_when_selling = data.fetch(:coef_stocks_when_selling)
		raise "coef_stocks_when_selling <= 0" unless inst.coef_stocks_when_selling > 0.0
...
coef_stocks_when_selling		#	If agent has money and there are some orders which are good enough to buy => buy one
	...
	def randomAct!
		...
		unless stock_available_to_buy.empty? or @money  < 1
			actions << :buy_stocks
		end		
		# sell stock action
